...
public class HttpGetContents {
	...
	public void setURI_F_GPS(String addr) {

		String strUri = CodeMaster.URLPTN2.replace("[addr]", addr);
		try {
			this.uri = new URI(strUri);
	...
	public void requestContents() throws Exception {
		...
		this.httpget = new HttpGet(this.uri.toString());
	...
	public String[] getOthers() {
		...
		for (int i = 0; i < x.length; i++) {
			is = this.strContents.indexOf(CodeMaster.pharseS_E[i][2])
					+ CodeMaster.pharseS_E[i][2].length();
			ie = this.strContents.indexOf(CodeMaster.pharseS_E[i][3]);
			if (is == -1 && ie > 0)
				is = ie - new Integer(CodeMaster.pharseS_E[i][1]);
			if (ie == -1 && is > 0)
				ie = is + new Integer(CodeMaster.pharseS_E[i][1]);
	...
	public static void main(String[] args) throws JAXBException {
		...
		/*
		 * String chiefOfNursery=new String(); String chiefOfNursery_keys=
		 * "<a href=\"#none\" title=\"설명글 닫기\" class=\"close\"><img src=\"http://img.childcare.go.kr/info/btn/btn_close1.gif\" alt=\"닫기\" /></a>\n							</span>\n						</span>\n					</th>\n					<td>"
		 * ;
		 * 
		 * String chiefOfNursery_keye=
		 * "</td>\n					<th>\n						<span class=\"tool_tip\">\n							<a href=\"#none\" title=\"설명글 열기"
		 * ; int is=0,ie=0;
		 */
		try {
			ci.setURI_F_GPS("%EC%97%BC%EC%B0%BD%EB%8F%99%20290%EB%B2%88%EC%A7%80");
			ci.requestContents();
			System.out.println(ci.getContents());

			/*
			 * is=chiefOfNursery.indexOf(chiefOfNursery_keys)+chiefOfNursery_keys
			 * .length(); ie=chiefOfNursery.indexOf(chiefOfNursery_keye);
			 * 
			 * System.out.println("s:"+is+"  e:"+ie);
			 */
